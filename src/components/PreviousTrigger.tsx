...
function PreviousTrigger({ currentIndex, setCurrentIndex, setSelectedAge, isPreviousVisible, setIsPreviousVisible, setIsNextVisible }: PreviousTriggerProps) {
  ...
  useEffect(() => {
    if (currentIndex !== null) {
      if (isPreviousVisible) {
        ...
        if (currentIndex - 1 >= 1) setPreviousValue(ages[currentIndex - 1])
        if (currentIndex - 2 >= 1) setPreviousDisplayValue(ages[currentIndex - 2]); else setPreviousDisplayValue('No Previous Groups')
      } else {
        // The button has not been clicked, so display value will be currentIndex - 1 and so will be the actual value
        if (currentIndex - 1 >= 1) setPreviousValue(ages[currentIndex - 1])
        if (currentIndex - 1 >= 1) setPreviousDisplayValue(ages[currentIndex - 1]); else setPreviousDisplayValue('No Previous Groups')
      }
    }
  }, [currentIndex])

  const handleClick = () => {
    if (previousDisplayValue === 'No Previous Groups' || currentIndex === null) {
      return;
    }
    ...
    if (previousDisplayValue === ages[currentIndex - 2]) {
      ...
      if (currentIndex - 1 >= 1) setPreviousDisplayValue(ages[currentIndex - 2]); else setPreviousDisplayValue('No Previous Groups')
    }
  }
